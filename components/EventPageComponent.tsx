...
export default function EventPageComponent({ data, logs }: {
  data: Event, logs: {
    id: number;
    timestamp: Date;
    level: string;
  ...
  return (
    <div className="p-8 text-white bg-[#0c0f1a] min-h-screen">
      {/* Title */}
      <h1 className="text-3xl font-bold mb-6">Event: {data.name}</h1>
      ...
      <div className="bg-[#141826] border border-[#1f2435] rounded-2xl shadow-lg p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <InfoRow label="Type" value={String(data.type)} />
          <InfoRow label="Once" value={data.once ? "Yes" : "No"} />
